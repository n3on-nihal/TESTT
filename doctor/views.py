...
from hospitalhome.models import User, Patient
...
# from hospitalhome.forms import UserCreationForm
from .forms import UserCreationForm,DoctorUserCreationForm
@csrf_exempt
def doctor_register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect("doctor-register-update")
...
@csrf_exempt
def doctor_register2(request):
    page = 'doctor-register-update'
    # form = DoctorUserCreationForm()
    form = DoctorForm()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Doctor account was updated!')
...
def doctor_login(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None and user.is_active and user.is_doctor:
            # User is a valid doctor, log them in
            login(request, user)
            return redirect('doctor-dashboard')
        else:
            # Invalid credentials or user is not a doctor
            messages.error(request, 'Invalid username or password for a doctor.')
            return redirect('login')  # Redirect to login page or appropriate page
    else:
        # Method is not POST, redirect to login page
...
def doctor_dashboard(request):
    # print(request,request.user, request.user.is_doctor, request.user.is_authenticated)
    if request.user.is_authenticated:
        if request.user.is_doctor:
            ...
            # context = {'doctor': request.user.name,
            #             'today_appointments': today_appointments, 'today_patient_count': today_patient_count, 'total_appointments_count': total_appointments_count, 'next_days_appointment': next_days_appointment, 'current_date': current_date_str, 'next_date': next_date_str}
...
def doctor_profile(request, pk):
    ...
    # doctor = Doctor_Information.objects.get(pk=1)
